...
class PPOAgent:
    ...
    def _get_distribution(self, state_tensor):
    ...
    def act(self, state, sensor_priority, location_priority):
        ...
        # Early warmup with heuristic decisions before policy becomes stable
        if self.samples_collected < self.warmup_steps:
            base_prob = 0.6
            ...
            transmission_prob = min(
                0.95,
                base_prob + priority_boost + urgency_boost + critical_boost + battery_boost,
            )
        ...
        with torch.no_grad():
            distribution, value = self._get_distribution(state_tensor)

            # Bias the logits towards transmitting when the situation is critical
            logits = distribution.logits.clone()
            transmit_bias = (
                (sensor_priority * location_priority) / 6.0
                + urgency * 1.2
                + is_critical * 2.0
                + battery_priority * 0.6
            )
            logits[0, 1] += transmit_bias
            logits[0, 0] += 0.2  # small bias to keep non-transmit viable
    ...
    def _process_memory(self):
        states = torch.FloatTensor([entry["state"] for entry in self.memory]).to(self.device)
        actions = torch.LongTensor([entry["action"] for entry in self.memory]).to(self.device)
        rewards = [entry["reward"] for entry in self.memory]
        dones = [entry["done"] for entry in self.memory]
        values = [entry["value"] for entry in self.memory]
        old_log_probs = torch.FloatTensor([entry["log_prob"] for entry in self.memory]).to(self.device)
    ...
    def update(self):
        ...
        states, actions, returns, advantages, old_log_probs = self._process_memory()
...
class MQTTSensorReader:
    ...
    def shutdown_gracefully(self):
        ...
        # Save PPO model
        torch.save(self.ppo_agent.model.state_dict(), f"ppo_model_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pth")
    ...
    def log_ppo_neural_network_parameters(self):
        """Log PPO neural network parameters and state"""
        timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        with open(self.performance_file, 'a', encoding='utf-8') as f:
            f.write(f"\n[{timestamp}] PPO NEURAL NETWORK PARAMETERS:\n")
            f.write("=" * 80 + "\n")
            ...
            f.write(f"- Samples Collected: {self.ppo_agent.samples_collected}\n")
            f.write(f"- Training Updates: {self.ppo_agent.training_steps}\n")
            f.write(f"- Pending Trajectory Length: {len(self.ppo_agent.memory)}\n")

            # Network weights statistics
            f.write("\nActor-Critic Weights Statistics:\n")
            for name, param in self.ppo_agent.model.named_parameters():
                if param.requires_grad:
                    ...
                    f.write(
                        f"- {name}: Mean={weight_mean:.6f}, Std={weight_std:.6f}, Min={weight_min:.6f}, Max={weight_max:.6f}\n"
                    )
            ...
            for name, param in self.ppo_agent.model.named_parameters():
                if param.requires_grad and param.grad is not None:
                    ...
                    f.write(
                        f"- {name} gradient: Mean={grad_mean:.6f}, Std={grad_std:.6f}, Norm={grad_norm:.6f}\n"
                    )
                else:
    ...
    def log_deadline_statistics(self):
        ...
        with open(self.performance_file, 'a', encoding='utf-8') as f:
            ...
            f.write(f"PPO Training Updates: {self.ppo_agent.training_steps}\n")
            f.write(f"Samples Collected: {self.ppo_agent.samples_collected}\n")

            # Log neural network parameters
            self.log_ppo_neural_network_parameters()
    ...
    def handle_sensor_data(self, topic, data):
        ...
        try:
            ...
            if time_since_last > (deadline / 2):
                ...
                # Get PPO decision
                action, log_prob, value_estimate = self.ppo_agent.act(
                    state,
                ...
                # Store experience for training when policy-driven
                done = remaining_time <= 0
                self.ppo_agent.remember(state, action, reward, done, log_prob, value_estimate)

                decision_type = "[PPO]" if log_prob is not None else "[HEURISTIC]"
...
def main():
    ...
    if mqtt_reader.connect_mqtt(broker_host, broker_port):
        ...
        mqtt_reader.log_to_file("Connected to MQTT broker successfully with PPO priority system (Silent Mode - Optimized)!")
